...
class PostListView(ListView):
    ...
    def get_queryset(self):
        ...
        # grab the post model all the objects there and filter out based on these conditions
...
# LoginRequiredMixin so we can use @login_required in a class based view
...
class PostDeleteView(LoginRequiredMixin, DeleteView):
    model = Post
    # with reverse_lazy, it waits until you deleted it to give you back the success_url that you are going to be going to
...
class DraftListView(LoginRequiredMixin,ListView):
    ...
    # the draft is not going to have a publication date
    def get_queryset(self):
...
# a decorator to make it required login
@login_required
#have the request and take the primary key
def add_comment_to_post(request, pk):
    post = get_object_or_404(Post, pk=pk)
    if request.method == 'POST':
        # meaning someone's actually filled in the form and enter something
        # the form is equal to the comment form passing that request
        form = CommentForm(request.POST)
        # if the form is valid meaning they didn't mess anything up
        if form.is_valid():
